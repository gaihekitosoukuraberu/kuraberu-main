        ...
        function handleLogin(event) {
            ...
            // ログインIDで権限を自動判定（管理者 or 営業）
            let loginSuccess = false;
            
            if (username === 'admin' && password === 'admin123') {
                currentUser = '田中太郎'; // 実際の名前
                userRole = 'admin';
                loginSuccess = true;
            } else if (username === 'sales' && password === 'sales123') {
                currentUser = '佐藤花子'; // 実際の名前
                userRole = 'sales';
                loginSuccess = true;
            }
            
            if (loginSuccess) {
                // ユーザー情報を保存（チェックボックスの状態で保存先を変更）
                const storage = rememberMe ? localStorage : sessionStorage;
                storage.setItem('franchiseUser', currentUser);
                storage.setItem('franchiseRole', userRole);
        ...
        function updateRoleDisplay() {
            ...
            document.getElementById('userRole').textContent = roleNames[userRole] + '権限';
        ...
        // 自動配信設定の保存
        function saveAutoDeliverySettings() {
            showNotification('保存完了', '自動配信設定を保存しました');
        }
        ...
        function addCustomQualification() {
            ...
            if (value) {
                ...
                tag.innerHTML = `
                    <span>${value}</span>
                    <button onclick="this.parentElement.remove(); autoSave()" class="ml-2 text-blue-600 hover:text-blue-800">
        ...
        function addCustomInsurance() {
            ...
            if (value) {
                ...
                tag.innerHTML = `
                    <span>${value}</span>
                    <button onclick="this.parentElement.remove(); autoSave()" class="ml-2 text-green-600 hover:text-green-800">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                        </svg>
                    </button>
        ...
        function handleMainVisualUpload(event) {
            ...
            if (file) {
                ...
                reader.onload = function(e) {
                    ...
                    showNotification('アップロード完了', 'メインビジュアルをアップロードしました');
                };
                ...
                if (file) {
                    ...
                    reader.onload = function(e) {
                        ...
                        imageDiv.innerHTML = `
                            ...
                            <button onclick="removeGalleryImage('${imageId}')" class="absolute top-1 right-1 bg-red-500 text-white p-1 rounded opacity-0 group-hover:opacity-100 transition">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        showNotification('アップロード完了', 'ギャラリー画像を追加しました');
        ...
        // 市区町村データ（サンプル）
        const cityData = {
            '埼玉県': ['さいたま市西区', 'さいたま市北区', 'さいたま市大宮区', 'さいたま市見沼区', 'さいたま市中央区', 'さいたま市桜区', 'さいたま市浦和区', 'さいたま市南区', 'さいたま市緑区', 'さいたま市岩槻区', '川越市', '川口市', '行田市', '秩父市', '所沢市', '飯能市', '加須市', '本庄市', '東松山市', '春日部市'],
            '千葉県': ['千葉市中央区', '千葉市花見川区', '千葉市稲毛区', '千葉市若葉区', '千葉市緑区', '千葉市美浜区', '銃橋市', '市川市', '松戸市', '野田市', '柏市'],
            '東京都': ['千代田区', '中央区', '港区', '新宿区', '文京区', '台東区', '墨田区', '江東区', '品川区', '目黒区', '大田区', '世田谷区', '渋谷区', '中野区', '杉並区', '豊島区', '北区', '荒川区', '板橋区', '練馬区', '足立区', '葛飾区', '江戸川区'],
            '神奈川県': ['横浜市鶴見区', '横浜市神奈川区', '横浜市西区', '横浜市中区', '横浜市南区', '川崎市川崎区', '川崎市幸区', '川崎市中原区', '相模原市', '横須賀市']
        };

        // 選択されたエリアを更新
        function updateSelectedAreas() {
            const checkboxes = document.querySelectorAll('input[name="prefecture"]:checked');

            // 上限チェック
            if (checkboxes.length > maxPrefectures) {
            ...
            if (selectedPrefectures.length > 0) {
                document.getElementById('selectedPrefectures').classList.remove('hidden');
                const listElement = document.getElementById('selectedPrefecturesList');
                listElement.innerHTML = selectedPrefectures.map(pref =>
                ...
                document.getElementById('selectedPrefectures').classList.add('hidden');
                document.getElementById('citySelection').classList.add('hidden');
            }

            // 会社情報に反映
            updateCompanyAreaDisplay();
        }
                    ...
                    cityData[pref].forEach(city => {
                        html += `
                            <label class="flex items-center p-2 bg-white border rounded hover:bg-gray-50 cursor-pointer">
                                <input type="checkbox" data-city="${city}" name="manual-city" value="${city}" class="mr-2" onchange="updateCompanyAreaDisplay(); autoSave();">
        ...
        // 優先エリアの切り替え
        function togglePriorityArea(prefecture, city) {
            // 優先エリア機能は削除されました
            // 通常の市区町村選択として動作
            const checkbox = document.querySelector(`input[data-city="${city}"]`);
            if (checkbox) {
        ...
        // 優先エリア表示を更新（互換性のため空関数として残す）
        function updatePriorityAreasDisplay() {
            // 優先エリア機能は削除されました
        }
        ...
        // 会社情報の入力フィールドに変更監視を追加
        document.addEventListener('DOMContentLoaded', () => {
            const companySection = document.getElementById('companySection');
            if (companySection) {
                // すべての入力要素に変更監視を追加
                companySection.querySelectorAll('input, textarea, select').forEach(element => {
                    element.addEventListener('change', autoSave);
                    element.addEventListener('input', () => {
                        clearTimeout(window.autoSaveTimer);
                        window.autoSaveTimer = setTimeout(autoSave, 1000); // 1秒後に自動保存
                    });
                });
...
請求書を発行してよろしいでしょうか？`;
        ...
        setInterval(() => {
            if (currentUser && Math.random() > 0.8) {
                ...
                showNotification('新規案件', '新しい案件が割り当てられました');
            }
        }, 30000);
