...
/**
 * 🔥 メイン関数：静的HTML生成（プレビュー完全一致版）
 */
function generateStaticHTML(data) {
  console.log('[generateStaticHTML] 🔥 プレビュー完全一致版でHTML生成開始');
  ...
  try {
    ...
    const phone = data['電話番号'] || '';
    const email = data['メールアドレス'] || data['営業用メールアドレス'] || '';
    const address = data['住所'] || '';
    const websiteUrl = data['ウェブサイトURL'] || '';
    const kuraberuScore = parseFloat(data['くらべるスコア'] || '4.2');

    // 編集パネルデータの取得
    const mainVisualUrl = data['メインビジュアル'] || '';

    // 🔥 プレビューシートから編集パネル設定を取得
    const mainVisualPositionX = parseFloat(data['メインビジュアル位置X'] || '50');
    const mainVisualPositionY = parseFloat(data['メインビジュアル位置Y'] || '50');
    const mainVisualZoom = parseFloat(data['メインビジュアルズーム'] || '100');
    const mainVisualBrightness = parseFloat(data['メインビジュアル明るさ'] || '50');
    const mainVisualTextColor = data['メインビジュアル文字色'] || '#1f2937';
    ...
    // レーダーチャート用評価データの処理
    const finalRatings = calculateRatings(data);
    ...
    const kuraberuScoreHtml = generateKuraberuScoreHtml(kuraberuScore, finalRatings);
...
function generateHeroSectionHtml(companyName, tagline, established, mainVisualUrl, visualSettings) {
  if (mainVisualUrl && mainVisualUrl.trim() !== '') {
    ...
    // 🔥 背景画像の位置とズームを計算
    const backgroundPosition = `${settings.positionX}% ${settings.positionY}%`;
    ...
    if (brightnessValue < 50) {
      // 暗く（黒オーバーレイ）
      const opacity = (50 - brightnessValue) / 50 * 0.6;
      overlayStyle = `background: linear-gradient(to bottom, rgba(0,0,0,0) 0%, rgba(0,0,0,${opacity}) 100%);`;
    } else if (brightnessValue > 50) {
      // 明るく（白オーバーレイ）
      const opacity = (brightnessValue - 50) / 50 * 0.6;
      overlayStyle = `background: linear-gradient(to bottom, rgba(255,255,255,0) 0%, rgba(255,255,255,${opacity}) 100%);`;
    } else {
    ...
    <div class="relative rounded-2xl overflow-hidden mb-12 min-h-[400px]">
        <div class="relative h-[500px] overflow-hidden">
            <img src="${mainVisualUrl}" alt="" class="w-full h-full object-cover" style="object-position: ${backgroundPosition}; transform: scale(${settings.zoom / 100});">
    ...
    <div id="preview-examples" class="mb-12" style="display: block;">
        ...
        <div class="relative">
            ...
            <div id="examplesCarousel" class="overflow-hidden" style="min-height: 400px;">
                <div id="examplesContainer" class="flex gap-4 transition-transform duration-300">
                    ${examplesHtml}
                </div>
            </div>
 ...
 */
function generateKuraberuScoreHtml(scoreValue, finalRatings) {
...
function generateRadarChartHTML(finalRatings) {
  ...
  const points = [
    ...
    { label: '対応スピード', value: finalRatings.schedule, angle: 90 },
    { label: 'アフターサポート', value: finalRatings.service, angle: 150 },
  ...
  return `
    <div class="bg-white rounded-xl shadow-lg border-2 border-amber-100 p-4 sm:p-5 lg:p-6">
        <h4 class="text-base sm:text-lg lg:text-xl font-bold text-gray-800 mb-4 text-center">項目別評価チャート</h4>
        <div class="flex justify-center">
            <svg width="400" height="400" viewBox="0 0 400 400" class="w-full max-w-sm">
                <!-- 背景グリッド -->
                <g id="radarGrid" transform="translate(200,200)">
                    ...
                    <circle cx="0" cy="0" r="100" fill="none" stroke="#6b7280" stroke-width="2"/>

                    <!-- 軸線 -->
                    <line x1="0" y1="-100" x2="0" y2="100" stroke="#e5e7eb" stroke-width="1"/>
                    <line x1="-100" y1="0" x2="100" y2="0" stroke="#e5e7eb" stroke-width="1"/>
                ...
                <g id="radarData" transform="translate(200,200)">
                    ...
                    ${points.map(point => {
                      ...
                      return `<circle cx="${x}" cy="${y}" r="4" fill="#f59e0b" stroke="#fff" stroke-width="2"/>`;
                    }).join('')}
                </g>
                ...
                <g id="radarLabels" transform="translate(200,200)">
                    ...
                    <text x="0" y="140" text-anchor="middle" class="text-xs sm:text-sm font-semibold fill-gray-700">対応スピード</text>
                    <text x="-115" y="65" text-anchor="end" class="text-xs sm:text-sm font-semibold fill-gray-700">アフターサポート</text>
                    <text x="-115" y="-25" text-anchor="end" class="text-xs sm:text-sm font-semibold fill-gray-700">顧客満足度</text>
                </g>
            </svg>
        </div>
    </div>`;
            ...
            <div class="bg-white border rounded-xl p-4 hover:shadow-lg transition">
                <div class="flex items-center mb-3">
                    ...
                    <div>
                        ...
                        <p class="text-xl font-bold text-blue-600">${phone}</p>
                        <p class="text-xs text-gray-500">受付時間：9:00-18:00（土日祝対応）</p>
                    </div>
                </div>
            </div>
            ` : ''}
            ...
            <div class="bg-white border rounded-xl p-4 hover:shadow-lg transition">
                <div class="flex items-center mb-3">
                    ...
                    <div class="overflow-hidden min-w-0">
                        ...
                        <p class="text-lg font-bold text-green-600 break-all">${email}</p>
                        <p class="text-xs text-gray-500">※24時間以内にご連絡いたします</p>
                    </div>
                </div>
            </div>
            ` : ''}
...
function generateGalleryHtml(galleryImages) {
  ...
  const galleryHtml = galleryImages.map((img, index) => `
    <div class="flex-shrink-0 w-72 aspect-square overflow-hidden rounded-lg cursor-pointer">
  ...
  return `
    <div class="mb-12">
        ...
        <div class="relative overflow-hidden">
            <div id="galleryContainer" class="flex gap-4 transition-transform duration-300">
                ${galleryHtml}
            </div>
        </div>
        <div class="flex justify-center gap-1 mt-4">
            ${galleryImages.map((_, index) => `
                <button onclick="goToGalleryImage(${index})" class="w-2 h-2 rounded-full transition ${index === 0 ? 'bg-blue-600' : 'bg-gray-300'}"></button>
            `).join('')}
        </div>
    </div>`;
}
...
function generateBranchMapsHtml(branchNames, branchAddresses) {
  ...
  const branchHtml = branchNames.map((name, index) => {
    ...
    return `
    <div class="mb-6">
        <h4 class="text-lg font-bold text-indigo-600 mb-3">${name}</h4>
        <div class="bg-white rounded-lg border shadow-sm p-4">
            <p class="text-gray-700 mb-3">📍 ${address}</p>
            <div class="bg-gray-100 rounded-lg h-48 flex items-center justify-center">
                <p class="text-gray-500">地図表示エリア</p>
            </div>
        </div>
    </div>`;
...
function parseConstructionExamples(data) {
  // 施工事例データの解析ロジック（スプレッドシートから）
  const examples = [];

  // 施工事例1
  if (data['施工事例1_タイトル'] || data['施工事例1_Before画像'] || data['施工事例1_After画像']) {
    examples.push({
      title: data['施工事例1_タイトル'] || 'sd f',
      age: data['施工事例1_築年数'] || '築15年',
      cost: data['施工事例1_費用'] || '150万円',
      description: data['施工事例1_説明'] || 'sd f',
      beforeImage: data['施工事例1_Before画像'] || '',
      afterImage: data['施工事例1_After画像'] || ''
    });
  }

  // 施工事例2
  if (data['施工事例2_タイトル'] || data['施工事例2_Before画像'] || data['施工事例2_After画像']) {
    examples.push({
      title: data['施工事例2_タイトル'] || 'あsだsd',
      age: data['施工事例2_築年数'] || '築20年',
      cost: data['施工事例2_費用'] || '200万円',
      description: data['施工事例2_説明'] || 'あsだsd',
      beforeImage: data['施工事例2_Before画像'] || '',
      afterImage: data['施工事例2_After画像'] || ''
    });
  }

  // 施工事例3
  if (data['施工事例3_タイトル'] || data['施工事例3_Before画像'] || data['施工事例3_After画像']) {
    examples.push({
      title: data['施工事例3_タイトル'] || 'あs',
      age: data['施工事例3_築年数'] || '築25年',
      cost: data['施工事例3_費用'] || '150万円',
      description: data['施工事例3_説明'] || 'あsだsだsd',
      beforeImage: data['施工事例3_Before画像'] || '',
      afterImage: data['施工事例3_After画像'] || ''
    });
  }

  return examples.filter(example => example.beforeImage || example.afterImage);
}
