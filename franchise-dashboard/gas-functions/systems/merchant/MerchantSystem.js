...
const MerchantSystem = {
    ...
    try {
      ...
      switch (action) {
        ...
        case 'generateStaticHTML':
          return this.generateStaticHTML(params);

        case 'generateStaticHTMLV2':
          return this.generateStaticHTMLV2(params);

        case 'testSimple':
          return this.testSimple(params);
  ...
  /**
   * 静的HTML生成
   * generateStaticHTML.jsの関数を呼び出す
   */
  generateStaticHTML: function(params) {
    try {
      console.log('[MerchantSystem] generateStaticHTML called with params:', JSON.stringify(params));

      // generateStaticHTML.jsの関数を呼び出し
      if (typeof generateStaticHTML === 'function') {
        const result = generateStaticHTML(params);
        console.log('[MerchantSystem] generateStaticHTML result type:', typeof result);

        // HTMLレスポンスの場合は直接HTMLContentServiceで返す
      ...
      console.error('[MerchantSystem] generateStaticHTML error:', error);
      return {
        success: false,
        error: error.toString()
      };
   ...
   * 🔥 テスト用：単純な関数
   */
  testSimple: function(params) {
    return { test: 'working', simple: true };
  },

  /**
   * 🔥 V2版：プレビュー完全一致 + Gemini完全除去
   */
  generateStaticHTMLV2: function(params) {
    return {
      success: true,
      message: 'V2 minimal working!',
