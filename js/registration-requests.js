...
console.log('[Registration Requests Module] v2.0 loaded - Display format fixed');
...
async function loadRegistrationRequestsData() {
    ...
    try {
        ...
        if (result && result.success !== false) {
            ...
            // フロントエンド側でステータスに基づいてフィルタリング（GAS側が正しく分類していない場合に備えて）
            const allData = currentRegistrationData;
            pendingRequests = allData.filter(item => {
                const approvalStatus = (item['承認ステータス'] || item.approvalStatus || '').toLowerCase();
                const status = (item['ステータス'] || item.status || '').toLowerCase();
                // 未審査判定：承認ステータスが空、または「申請中」「未審査」
                return !approvalStatus || approvalStatus === '' ||
                       approvalStatus === '申請中' || approvalStatus === '未審査' ||
                       status === '申請中' || status === '未審査';
            });

            approvedRequests = allData.filter(item => {
                const approvalStatus = (item['承認ステータス'] || item.approvalStatus || '').toLowerCase();
                // 承認済み判定
                return approvalStatus === '承認済み' || approvalStatus === '承認' || approvalStatus === 'approved';
            });

            rejectedRequests = allData.filter(item => {
                const approvalStatus = (item['承認ステータス'] || item.approvalStatus || '').toLowerCase();
                const status = (item['ステータス'] || item.status || '').toLowerCase();
                // 却下判定
                return approvalStatus === '却下' || approvalStatus === '却下済み' ||
                       status === '却下' || status === '却下済み' || approvalStatus === 'rejected';
            });
