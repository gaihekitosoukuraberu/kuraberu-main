...
function postToMail($arr){
	...
	foreach($arr as $key => $val) {
		...
		// }
	    if($out != "confirm_submit" && $key != "httpReferer") {
			if($key != '当社規定のプライバシーポリシー・利用規約へ同意する。') {
				$resArray .= "【 ".h($key)." 】 ".h($out)."\n";
			}
		}
	}
...
function mailToAdmin($arr,$subject,$mailFooterDsp,$mailSignature,$encode,$confirmDsp){
	// $adminBody= $subject."\n\n";
	$adminBody .="お問い合わせフォームからお問い合わせがありました。\n内容は以下のとおりです。\n\n";
...
function getGasUrlFromEnvLoader(){
	// マスターenv-loader.jsのパス（/js/env-loader.js）
	$envLoaderPath = __DIR__ . '/../js/env-loader.js';

	// フォールバックURL（ファイル読み込み失敗時）- V1851 @1749（全員アクセス可能 + LPContactSystem対応）
	$fallbackUrl = 'https://script.google.com/macros/s/AKfycbz_LISh70hJYWbz8KO54DpMFXijjSuTEgaChXma2yGjebSQcKb4c-ca0C6n5bk64CMeiQ/exec';
