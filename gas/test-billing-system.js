...
/**
 * 配信管理シートの「加盟店ID」カラムを確認（ドライラン）
 * 会社名が入っている場合、正しい店舗IDに修正が必要
 */
function checkDeliverySheetMerchantIds() {
  console.log('========== 配信管理シート 加盟店ID確認 ==========');
  ...
  if (!deliverySheet || !masterSheet) {
    console.log('エラー: シートが見つかりません');
    return;
  }

  // 加盟店マスタから「会社名→店舗ID」マッピング作成
  const masterData = masterSheet.getDataRange().getValues();
  const masterHeaders = masterData[0];
  const masterIdIdx = masterHeaders.indexOf('加盟店ID');
  const companyNameIdx = masterHeaders.indexOf('会社名');

  console.log('加盟店マスタ: 加盟店ID=' + masterIdIdx + '列目, 会社名=' + companyNameIdx + '列目');

  const companyToId = {};  // 会社名 → 店舗ID
  const idSet = new Set(); // 有効な店舗IDのセット

  for (let i = 1; i < masterData.length; i++) {
    const id = masterData[i][masterIdIdx];
    const company = masterData[i][companyNameIdx];
    if (id && company) {
      companyToId[company] = id;
      idSet.add(id);
    }
  }

  console.log('マスタ登録数:', Object.keys(companyToId).length);

  // 配信管理シートをチェック
  const deliveryData = deliverySheet.getDataRange().getValues();
  const deliveryHeaders = deliveryData[0];
  const merchantIdIdx = deliveryHeaders.indexOf('加盟店ID');

  console.log('配信管理: 加盟店ID=' + merchantIdIdx + '列目');

  const issues = [];
  const stats = { valid: 0, companyName: 0, unknown: 0 };

  for (let i = 1; i < deliveryData.length; i++) {
    const currentValue = deliveryData[i][merchantIdIdx];
    if (!currentValue) continue;

    if (idSet.has(currentValue)) {
      // 正しい店舗ID
      stats.valid++;
    } else if (companyToId[currentValue]) {
      // 会社名が入っている → 修正が必要
      stats.companyName++;
      issues.push({
        row: i + 1,
        current: currentValue,
        shouldBe: companyToId[currentValue]
      });
    } else {
      // どちらにも該当しない
      stats.unknown++;
  ...
  console.log('\n--- 統計 ---');
  console.log('正しい店舗ID:', stats.valid);
  console.log('会社名が入っている（要修正）:', stats.companyName);
  console.log('不明:', stats.unknown);

  if (issues.length > 0) {
    console.log('\n--- 修正が必要な行（最大20件表示）---');
    issues.slice(0, 20).forEach(issue => {
      console.log('行' + issue.row + ': 「' + issue.current + '」→「' + issue.shouldBe + '」');
    });
    if (issues.length > 20) {
  ...
  console.log('\n========== 完了 ==========');
  console.log('修正を実行するには fixDeliverySheetMerchantIds() を実行してください');

  return { stats, issues };
}

/**
 * 配信管理シートの「加盟店ID」カラムを修正（実行）
 * ※ 実際にシートを更新します
 */
function fixDeliverySheetMerchantIds() {
  console.log('========== 配信管理シート 加盟店ID修正 ==========');
  ...
  const deliveryHeaders = deliveryData[0];
  const merchantIdIdx = deliveryHeaders.indexOf('加盟店ID');
  const merchantIdCol = merchantIdIdx + 1; // 1-indexed
  ...
  for (let i = 1; i < deliveryData.length; i++) {
    ...
    // 会社名が入っている場合のみ修正
    if (!idSet.has(currentValue) && companyToId[currentValue]) {
      const newValue = companyToId[currentValue];
      const rowNum = i + 1;

      deliverySheet.getRange(rowNum, merchantIdCol).setValue(newValue);
      fixes.push({ row: rowNum, from: currentValue, to: newValue });
      fixedCount++;
    }
  }
  ...
  if (fixes.length > 0) {
    ...
    fixes.slice(0, 20).forEach(fix => {
      console.log('行' + fix.row + ': 「' + fix.from + '」→「' + fix.to + '」');
    });
    if (fixes.length > 20) {
      console.log('... 他 ' + (fixes.length - 20) + ' 件');
    }
  }
