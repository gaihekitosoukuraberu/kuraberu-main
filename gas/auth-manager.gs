...
// SECRET_KEY: プロパティから読み込み
const SECRET_KEY = PropertiesService.getScriptProperties().getProperty('SECRET_KEY') || 'kuraberu-2024-secret-key';
...
function generateFirstLoginUrl(merchantId) {
  ...
  // URL生成
  const baseUrl = 'https://gaihekikuraberu.com/merchant-portal/first-login.html';
...
function savePassword(merchantId, plainPassword) {
  ...
  if (rowIndex > 0) {
    ...
    // 新規追加
    const merchantSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('加盟店登録');
...
function generatePasswordResetUrl(email) {
  ...
  return `https://gaihekikuraberu.com/merchant-portal/reset-password.html?data=${payload}&sig=${signature}`;
}
