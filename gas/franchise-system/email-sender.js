...
<body>
  <div class="container">
    ...
    <div class="footer">
      <p>
        ...
        サポートデスク: info@gaihekikuraberu.com<br>
        営業時間: 平日 9:00-18:00
      </p>
  ...
  try {
    // MailAppを使用（fromはGoogleアカウントのメールアドレスになる）
    MailApp.sendEmail({
      to: email,
      subject: subject,
      htmlBody: htmlBody,
    ...
    console.log(`初回メール送信成功: ${email} (${merchantId})`);
  } catch (mailError) {
...
function resendWelcomeEmail(merchantId) {
  ...
  try {
    ...
    if (merchant && merchant.length > 22) {
      ...
      sendWelcomeEmail(email, companyName, newUrl);
