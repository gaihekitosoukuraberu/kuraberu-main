...
const RankingSystem = {
    ...
    try {
      ...
      if (registerSheet) {
        ...
        if (regLastRow >= 2) {
          ...
          // V2041: K列（index 10）を電話番号列として固定使用
          // ヘッダー名に依存せず、K列を直接参照
          const phoneIdx = 10;  // K列 = index 10 (0ベース)
          console.log('[RankingSystem] 電話番号列: K列 (index 10) を使用');
          console.log('[RankingSystem] K列ヘッダー名:', regHeaders[10]);
   ...
   */
  /**
   * V1913: 住所を加盟店登録から加盟店マスタに同期
   */
  syncAddressToMaster: function() {
    console.log('[RankingSystem] 住所同期開始');
    ...
    try {
      ...
      // ヘッダーから列インデックスを取得
      const companyNameColIndex = registerHeaders.indexOf('会社名');
      const addressColIndex = registerHeaders.indexOf('住所');

      console.log('[V1913-DEBUG] 加盟店登録シート - 列インデックス:', {
        companyName: companyNameColIndex,
      ...
      if (addressColIndex === -1) {
        console.error('[RankingSystem] 加盟店登録シートに「住所」列が見つかりません');
        return { success: false, error: '加盟店登録シートに「住所」列が見つかりません' };
      }
      ...
      for (let i = 1; i < registerData.length; i++) {
        ...
        const address = String(registerData[i][addressColIndex] || '').trim();

        dataMap[companyName] = {
          address: address
        };

        console.log('[V1913-DEBUG] 加盟店登録データ:', companyName, '住所:', address || '(空)');
      }
      ...
      const masterHeaders = masterSheet.getRange(1, 1, 1, masterSheet.getLastColumn()).getValues()[0];
      const masterCompanyNameColIndex = masterHeaders.indexOf('会社名');
      const masterAddressColIndex = masterHeaders.indexOf('住所');

      console.log('[V1913-DEBUG] 加盟店マスタ - 列インデックス:', {
        companyName: masterCompanyNameColIndex,
        address: masterAddressColIndex
      });
      ...
      if (masterAddressColIndex === -1) {
        console.error('[RankingSystem] 加盟店マスタに「住所」列が見つかりません');
        return { success: false, error: '加盟店マスタに「住所」列が見つかりません' };
      }
      ...
      for (let i = 2; i <= masterLastRow; i++) {
        ...
        if (companyName && dataMap[companyName]) {
          ...
          // 住所を同期
          masterSheet.getRange(i, masterAddressColIndex + 1).setValue(data.address);

          console.log('[V1913-DEBUG] ✅ マスタ更新:', companyName, '→ 住所:', (data.address || '(空)'));
          updatedCount++;
        } else if (companyName) {
          console.log('[RankingSystem] ⚠️ 加盟店登録に存在しない:', companyName);
          notFoundCount++;
  ...
  handle: function(params) {
    ...
    // V1913: 住所同期エンドポイント
    if (action === 'syncAddress') {
      return this.syncAddressToMaster();
    }
