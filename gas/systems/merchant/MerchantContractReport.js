...
var MerchantContractReport = {
  ...
  updateCaseStatus: function(params) {
    ...
    // V2207: StatusDefinitionsと同期した有効ステータス
    const validStatuses = [
      // 追客中
      '新着', '未アポ', 'アポ済', '現調済', '見積提出済',
      // 成約済（入金×工事の組み合わせ）
      '入金予定・未着工', '入金予定・施工中', '入金予定・工事済',
      '入金済・未着工', '入金済・施工中', '完了',
      // 終了（失注・キャンセル）
      '現調前キャンセル', '現調後失注', '他社契約済', '別加盟店契約済', '顧客クレーム',
      // 後方互換用（旧ステータス）
      '成約', '入金予定', '入金済', 'クレーム', '未着工', '施工中', '工事済', '入金済・工事済'
    ];
  ...
  _getMostAdvancedStatus: function(statuses) {
    // V2207: StatusDefinitionsと同期した優先順位（数字が大きいほど進んでいる）
    const statusOrder = {
      // 追客中
      '新着': 1,
      '未アポ': 2,
