...
function startAIHearing(params) {
  try {
    ...
    if (!OPENROUTER_API_KEY) {
      ...
      return {
        success: false,
        error: 'システムエラー: OpenRouter APIキーが設定されていません。管理者にお問い合わせください。'
      };
    }
    ...
    if (!aiExtraction.success) {
      ...
      return {
        success: false,
        error: 'OpenRouter API抽出エラー: ' + aiExtraction.error
      };
    }
...
function extractCompanyInfoWithDeepSeek(companyName) {
  try {
    ...
    if (!searchResults || searchResults.length < 50) {
      ...
      return {
        success: false,
        error: companyName + ' の検索結果が見つかりませんでした',
        candidates: []
      };
    }
...
- 推測や生成はせず、実際の情報のみを使用`;
    ...
    if (!deepSeekResponse.success) {
      return {
        success: false,
        error: 'DeepSeek API呼び出し失敗: ' + deepSeekResponse.error,
        candidates: []
      };
    }
    ...
    try {
      ...
      Logger.log('❌ JSON解析エラー: ' + parseError.message);
    }
    ...
    Logger.log('❌ extractCompanyInfoWithDeepSeek エラー: ' + error.message);
    return {
      success: false,
      error: error.message,
      candidates: []
