 ...
 */
function notifyNewFranchiseRegistration(franchiseId, franchiseData) {
  try {
    const message = `🏢 **新規加盟店登録**

🆔 **加盟店ID**: ${franchiseId}
🏢 **会社名**: ${franchiseData.companyName || '未設定'}
👤 **代表者**: ${franchiseData.representativeName || '未設定'}
📧 **メール**: ${franchiseData.email || '未設定'}
📞 **電話**: ${franchiseData.phone || '未設定'}
📅 **登録日時**: ${Utilities.formatDate(new Date(), 'JST', 'yyyy/MM/dd HH:mm:ss')}

確認・承認をお願いします。`;

    return sendIntegratedNotification('new_franchise', message, {
      username: '加盟店登録通知',
      icon: ':office:',
      channel: '#加盟店管理'
    });
    ...
    Logger.log(`❌ 新規加盟店登録通知エラー: ${error.message}`);
    return { success: false, error: error.message };
  }
}
 ...
 * @param {string} franchiseId 加盟店ID
 */
function notifyAssignmentComplete(caseId, franchiseId) {
  try {
    const message = `📋 **案件振り分け完了**

🆔 **案件ID**: ${caseId}
🏢 **担当加盟店**: ${franchiseId}
📅 **振り分け日時**: ${Utilities.formatDate(new Date(), 'JST', 'yyyy/MM/dd HH:mm:ss')}

案件の対応を開始してください。`;

    return sendIntegratedNotification('assignment_complete', message, {
      username: '案件振り分け通知',
      icon: ':clipboard:',
      channel: '#案件管理'
    });
    ...
    Logger.log(`❌ 案件振り分け完了通知エラー: ${error.message}`);
    return { success: false, error: error.message };
  }
}
 ...
 * @returns {Object} 保存結果
 */
function saveFranchiseData(franchiseId, data) {
  try {
    const SPREADSHEET_ID = PropertiesService.getScriptProperties().getProperty('SPREADSHEET_ID');
    if (!SPREADSHEET_ID) {
    ...
    if (!sheet) {
      ...
      const headers = ['加盟店ID', '会社名', '代表者名', 'メールアドレス', '電話番号', '住所', '登録日', 'ステータス'];
    ...
    Logger.log(`✅ 加盟店データ保存完了: ${franchiseId}`);
    
    return { success: true, franchiseId: franchiseId };
    
  } catch (error) {
    Logger.log(`❌ 加盟店データ保存エラー: ${error.message}`);
    return { success: false, error: error.message };
  }
}
 ...
 * @returns {Object} 保存結果
 */
function saveCancelRequest(cancelData) {
  try {
    const SPREADSHEET_ID = PropertiesService.getScriptProperties().getProperty('SPREADSHEET_ID');
    if (!SPREADSHEET_ID) {
      throw new Error('SPREADSHEET_ID未設定');
    ...
    if (!sheet) {
      ...
      const headers = ['申請ID', '案件ID', '申請者', 'キャンセル理由', '申請日時', 'ステータス'];
      sheet.getRange(1, 1, 1, headers.length).setValues([headers]);
    }
    
    const newRow = [
      cancelData.requestId,
      cancelData.caseId || '',
      cancelData.applicant || '',
      cancelData.reason || '',
      cancelData.timestamp || new Date(),
      '申請中'
    ];
    
    sheet.appendRow(newRow);
    Logger.log(`✅ キャンセル申請保存完了: ${cancelData.requestId}`);
    
    return { success: true, requestId: cancelData.requestId };
 ...
 */
function assignCaseToFranchise(caseId, franchiseId) {
  try {
    const SPREADSHEET_ID = PropertiesService.getScriptProperties().getProperty('SPREADSHEET_ID');
    if (!SPREADSHEET_ID) {
      throw new Error('SPREADSHEET_ID未設定');
    ...
    const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    let sheet = ss.getSheetByName('案件振り分け履歴');
    
    if (!sheet) {
      sheet = ss.insertSheet('案件振り分け履歴');
      const headers = ['振り分けID', '案件ID', '加盟店ID', '振り分け日時', 'ステータス'];
      sheet.getRange(1, 1, 1, headers.length).setValues([headers]);
    }
    ...
    sheet.appendRow(newRow);
    Logger.log(`✅ 案件振り分け完了: ${caseId} → ${franchiseId}`);
    
    return { success: true, assignmentId: assignmentId };
    
  } catch (error) {
    Logger.log(`❌ 案件振り分けエラー: ${error.message}`);
    return { success: false, error: error.message };
  }
}
 ...
 * @returns {Object} 保存結果
 */
function saveInquiry(inquiryData) {
  try {
    const SPREADSHEET_ID = PropertiesService.getScriptProperties().getProperty('SPREADSHEET_ID');
    if (!SPREADSHEET_ID) {
      throw new Error('SPREADSHEET_ID未設定');
    ...
    if (!sheet) {
      ...
      const headers = ['問い合わせID', 'ユーザーID', '問い合わせ内容', 'カテゴリ', '受付日時', 'ステータス'];
      sheet.getRange(1, 1, 1, headers.length).setValues([headers]);
    }
    ...
    const newRow = [
      ...
      inquiryData.timestamp || new Date(),
      '受付済み'
    ];
    
    sheet.appendRow(newRow);
    Logger.log(`✅ 問い合わせ保存完了: ${inquiryData.inquiryId}`);
    
    return { success: true, inquiryId: inquiryData.inquiryId };
    
  } catch (error) {
    Logger.log(`❌ 問い合わせ保存エラー: ${error.message}`);
    return { success: false, error: error.message };
  }
}