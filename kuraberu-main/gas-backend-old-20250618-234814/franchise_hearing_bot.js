...
function initializeFranchiseHearingDatabase() {
  try {
    ...
    initializedSheets.push(FRANCHISE_HEARING_SHEETS.QUESTIONS);
...
function initializeQuestionListSheet(ss) {
  const sheetName = FRANCHISE_HEARING_SHEETS.QUESTIONS;
...
/**
 * 加盟店ヒアリングBOT専用 - Webhook受信ハンドラー
 * notify_fixed.gs のdoPost()から呼び出される
 * 
 * @param {Object} e POST リクエスト
 * @returns {Object} 処理結果
 */
function doPostFranchiseHearing(e) {
  try {
