/**
 * 📂 ファイル名: initTest.gs
 * 🎯 内容: 外壁塗装くらべるAI - 統合初期化・テストシステム
 * - 総合システムチェック（スクリプトプロパティ検証含む）
 * - 初期化テスト・ヘルスチェック・デバッグ機能統合
 * - 重複機能を整理し、完全版として提供
 * ✅ claude_sheetCheckTest() により最も包括的なシステムチェックが可能
 * 📌 機能保全移植版 - 既存機能完全維持・統合最適化
 */

/**
 * 統合総合システムチェック - 最も包括的なメイン関数
 * スクリプトプロパティ検証を含む完全版システムチェック
 * 
 * @returns {Object} 包括的チェック結果
 */
function claude_sheetCheckTest() {
  try {
    console.log('=== Claude Code 統合システム総合チェック開始 ===');
    ...
    console.log('0️⃣ スクリプトプロパティ整合性確認...');
    try {
      claude_checkScriptProperties();
      results.scriptProperties = true;
    } catch (error) {
      results.errors.push(`スクリプトプロパティ: ${error.toString()}`);
    }
    ...
    console.log('=== Claude Code 統合システム総合チェック結果 ===');
    console.log(`スクリプトプロパティ: ${results.scriptProperties ? '✅' : '❌'}`);
    ...
    const overallSuccess = results.scriptProperties && results.githubSync && 
                          results.sheetStructure && results.dataAccess && results.coreSheets;
    ...
    console.error('統合システム総合チェックエラー:', error.toString());
    claude_logEvent('ERROR', '統合システムチェック', '総合チェック', error.toString());
    throw error;
  }
}
...
/**
 * 統合初期化・テストシステムのテスト（模擬実行）
 * 
 * @returns {Object} テスト結果
 */
function testIntegratedInitSystem() {
  console.log('🧪 統合初期化・テストシステムテスト開始');
  
  try {
    console.log('--- クイックヘルスチェックテスト ---');
    const healthCheck = claude_quickHealthCheck();
    
    console.log('--- 重要シート存在確認テスト ---');
    const coreSheets = claude_checkCoreSheets();
    
    console.log('--- データアクセステスト ---');
    const dataAccess = claude_testDataAccess();
    
    console.log('--- 統合総合システムチェックテスト ---');
    const systemCheck = claude_sheetCheckTest();
    
    console.log('✅ 統合初期化・テストシステムテスト完了');
    
    const testSummary = {
      healthCheck: healthCheck,
      coreSheets: coreSheets,
      dataAccess: dataAccess,
      integratedSystemCheck: systemCheck.scriptProperties && systemCheck.githubSync && 
                            systemCheck.sheetStructure && systemCheck.dataAccess && systemCheck.coreSheets
    };
    ...
    return {
      ...
      testResults: {
        ...
        coreSheets: coreSheets,
        dataAccess: dataAccess,
        integratedSystemCheck: systemCheck
      }
    };
    
  } catch (error) {
    console.error('❌ 統合初期化・テストシステムテストエラー:', error);
