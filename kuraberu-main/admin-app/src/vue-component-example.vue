<template>
  <div class="gas-api-demo">
    ...
    <div v-if="error" class="result-box error">
      ...
      <!-- CORS エラーの場合はJSONPフォールバック提案 -->
      <div v-if="error.includes('CORS')" class="fallback-suggestion">
        <p><strong>💡 CORS エラーが発生しました。JSONPモードを試してみてください：</strong></p>
        <button @click="handleTestConnectionJsonp" class="btn-secondary">
          JSONP で接続テスト
        </button>
      </div>
    </div>
...
export default {
  ...
  setup() {
    ...
    const handleTestConnectionJsonp = async () => {
      try {
        const result = await gasApiService.callFunctionJsonp('testConnection');
        connectionResult.value = result;
