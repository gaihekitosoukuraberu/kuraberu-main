...
function submitFranchiseRegistration(registrationData) {
  try {
    ...
    console.log('🔍 DEBUG: このIDがスプレッドシートと表示の両方で使われるべき');
    ...
    // データ配列を作成（A-AG列まで33列）
    console.log('🔍 DEBUG: 使用するfranchiseId:', franchiseId);
    console.log('🔍 DEBUG: franchiseIdの型:', typeof franchiseId);
    console.log('🔍 DEBUG: このIDがA列に入ります');
    ...
    const rowData = [
      ...
      (function() {
        console.log('🔍 DEBUG R列: constructionAreas:', registrationData.constructionAreas);
        const result = decompressServiceList(registrationData.constructionAreas) || '';
        console.log('🔍 DEBUG R列: 展開結果:', result);
        return result;
      })(),           // R: 施工箇所
      (function() {
        console.log('🔍 DEBUG S列: specialServices:', registrationData.specialServices);
        const result = decompressServiceList(registrationData.specialServices) || '';
        console.log('🔍 DEBUG S列: 展開結果:', result);
        return result;
      })(),             // S: 特殊対応項目
