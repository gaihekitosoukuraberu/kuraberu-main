/**
 * キャンセル申請システム用テストデータ作成
 * ユーザー登録シートにテストデータを追加
 */

function createCancelTestData() {
  console.log('===== キャンセル申請テストデータ作成開始 =====');
  ...
  // 「配信先業者一覧」カラムが存在しない場合は追加
  if (headers.indexOf('配信先業者一覧') === -1) {
    console.log('⚠️  「配信先業者一覧」カラムが存在しないため追加します');
    const lastCol = userSheet.getLastColumn();
    userSheet.getRange(1, lastCol + 1).setValue('配信先業者一覧');
    // ヘッダーを再取得
    headers = userSheet.getRange(1, 1, 1, userSheet.getLastColumn()).getValues()[0];
    console.log('✅ 「配信先業者一覧」カラムを追加しました（カラム' + (lastCol + 1) + '）');
  }
  ...
  const testData = [
    {
      ...
      配信ステータス: '配信済み',
      配信日時: sixDaysAgo,
      配信先業者一覧: 'FR251112004600',
      管理ステータス: '配信後未成約',
      担当者名: 'テスト営業A',
      架電履歴: JSON.stringify([
        { date: sixDaysAgo, type: '電話' },
        { date: sixDaysAgo, type: '電話' },
        { date: sixDaysAgo, type: 'SMS' },
        { date: sixDaysAgo, type: '電話' },
        { date: sixDaysAgo, type: 'SMS' },
        { date: sixDaysAgo, type: '電話' },
        { date: sixDaysAgo, type: 'SMS' },
  ...
  testData.forEach((data, index) => {
    ...
    addedCount++;
    console.log(`✅ テストデータ${index + 1}を追加: ${data.会社名} (行${newRow})`);
  });
  ...
  console.log(`\nテストケース:`);
  console.log('1. テスト株式会社A - フォロー履歴充分（電話5回、SMS3回）→ キャンセル申請可能');
  console.log('2. テスト株式会社B - フォロー履歴不足（電話1回、SMS0回）→ バリデーションエラーになる想定');
  console.log('3. テスト株式会社C - 期限延長承認済み → 延長後の期限が適用される');
}
...
function deleteTestData() {
  ...
  const companyNameCol = headers.indexOf('会社名') + 1;

  if (companyNameCol === 0) {
    console.error('❌ 会社名カラムが見つかりません');
    return;
  }
  ...
  for (let i = lastRow; i >= 2; i--) {
    const companyName = userSheet.getRange(i, companyNameCol).getValue();
    if (companyName && companyName.toString().startsWith('テスト株式会社')) {
      userSheet.deleteRow(i);
      deletedCount++;
      console.log(`削除: ${companyName} (行${i})`);
    }
  }
  ...
  const extensionSheet = ss.getSheetByName('キャンセル期限延長申請');
  if (extensionSheet) {
    const extHeaders = extensionSheet.getRange(1, 1, 1, extensionSheet.getLastColumn()).getValues()[0];
    const extCompanyCol = extHeaders.indexOf('顧客会社名') + 1;
    const extLastRow = extensionSheet.getLastRow();

    for (let i = extLastRow; i >= 2; i--) {
      const companyName = extensionSheet.getRange(i, extCompanyCol).getValue();
      if (companyName && companyName.toString().startsWith('テスト株式会社')) {
        extensionSheet.deleteRow(i);
        console.log(`削除（期限延長）: ${companyName}`);
      }
    }
  }

  console.log(`\n===== 完了 =====`);
  console.log(`削除したテストデータ: ${deletedCount}件`);
}
