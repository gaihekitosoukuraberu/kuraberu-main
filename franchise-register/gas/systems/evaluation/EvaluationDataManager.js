  ...
  generateAIEvaluation: function(companyName, googleRating, nurikaeRating, reshopnaviRating) {
    ...
    try {
      ...
      if (!openRouterKey) {
        ...
        return '複数の評価指標で安定した実績が確認されており、総合的な施工品質と顧客対応力に優れた事業者です。';
      }

      // 総合スコアを計算（他社サイト名は絶対に出さない）
      const avgScore = ((googleRating + nurikaeRating + reshopnaviRating) / 3).toFixed(1);

      // DeepSeek用プロンプト（AI分析風、落ち着いたトーンでロジカルに、さりげなく上げ↑）
      const prompt = `あなたは外壁塗装業界の評価分析AIです。以下の会社について、複数の評価指標を分析した結果を基に、客観的かつ丁寧なトーンで総合評価コメントを200文字以内で生成してください。

**絶対に守るべきルール:**
1. **AIらしく落ち着いた丁寧なトーン**（「〜と評価されています」「〜が確認できます」など）
2. **ロジカルな分析要素**を含める（例：「施工品質と対応力の両面で」「複数の評価軸で安定した」「技術力とコストバランスに優れ」）
3. **さりげなく上げ↑**（ポジティブだが過度に煽らない、自然な高評価）
4. **100社あれば100通り**の違う表現にする（分析の着眼点を変える、パターン化厳禁）
5. **他社サイト名（Google、ヌリカエ、リショップナビ等）は絶対に出さない**
6. 「当社AI分析」「総合的な評価」「複数指標の分析結果」など、くらべる独自の分析であることを示唆
7. 会社名の特徴（地域、規模、専門性など）を活かして独自性を出す
8. 語尾は「〜です」「〜ます」「〜が見られます」「〜と判断されます」など丁寧語
9. 200文字以内で詳細に分析

**悪い例（イケイケ営業調）:**
「地域No.1の実績と信頼！今すぐ無料相談で安心の施工を！」

**良い例（AI分析風）:**
「施工品質と顧客対応の両面で高い評価を獲得しており、コストバランスにも優れた総合力の高い事業者です。特に技術力の安定性と対応スピードの速さが強みとして確認されています。」
「複数の評価軸で安定した実績が確認でき、特に技術力と対応スピードに強みを持つ業者と分析されます。アフターサポート体制も充実しており、長期的な信頼関係を構築できる事業者といえます。」

**評価データ:**
会社名: ${companyName}
総合評価スコア: ${avgScore}/5.0
評価項目数: 6項目（コスト、対応力、技術、サポート、スピード、満足度）

**出力形式:** AI分析コメントのみを出力（説明不要、引用符不要、200文字以内）`;
      ...
      if (result.choices && result.choices.length > 0) {
        ...
        console.log('[EvaluationData] DeepSeek AI評価生成成功:', aiEvaluation);
        return aiEvaluation;
      }
      ...
      return '施工品質と顧客対応の両面で高い評価を獲得しており、コストバランスにも優れた総合力の高い事業者です。';
    }
  },
