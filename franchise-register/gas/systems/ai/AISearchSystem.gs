...
const AISearchSystem = {
  ...
  analyzeWithAI: function(searchResults, companyName, apiKey) {
    ...
    var userMessage =
      ...
      "【最重要フィールド（絶対に抽出すること）】\n" +
      "- representative：代表者の氏名（**最優先**。肩書きは除外。例: 山田太郎）\n" +
      "  ※「代表取締役」「代表」「社長」「CEO」などのキーワード付近を徹底的に探索\n" +
      "  ※会社概要、企業情報、代表挨拶、沿革、スタッフ紹介などのページから抽出\n" +
      "- representative_kana：代表者のカタカナ（必須。無ければひらがなをカタカナに変換）\n" +
      "- established：設立年月（**最優先**。必ず西暦で「YYYY年M月」形式。例: 2000年10月）\n" +
      "  ※「設立」「創業」「会社設立」などのキーワード付近を徹底的に探索\n" +
      "  ※昭和・平成・令和の和暦は西暦に変換すること\n" +
      "  ※本文に明記がなければ空文字（憶測禁止）\n\n" +
      "【必須フィールド】\n" +
